...
static le_result_t wlanSTATestSetWpa2Psk() {
    taf_wlanSta_WlanSTARef_t staRef = taf_wlanSta_GetWlanSTA(g_StaID, g_StaIntf);
    if (!staRef) {
        fprintf(stderr, "taf_wlanSta_GetWlanSTA failed\n");
        return LE_FAULT;
    ...
    std::string ssid(le_arg_GetArg(1));
    ...
    {
        psk = std::string(le_arg_GetArg(2));
    }
...
static le_result_t wlanSTATestConnect() {
    taf_wlanSta_WlanSTARef_t staRef = taf_wlanSta_GetWlanSTA(g_StaID, g_StaIntf);
    if (!staRef) {
        fprintf(stderr, "taf_wlanSta_GetWlanSTA failed\n");
        return LE_FAULT;
...
static le_result_t wlanSTATestDisconnect() {
    taf_wlanSta_WlanSTARef_t staRef = taf_wlanSta_GetWlanSTA(g_StaID, g_StaIntf);
    if (!staRef) {
        fprintf(stderr, "taf_wlanSta_GetWlanSTA failed\n");
        return LE_FAULT;
