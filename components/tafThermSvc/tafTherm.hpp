...
typedef struct
{
    taf_therm_TripType_t tripType; // UNKNOWN
    uint32_t threshold;
    uint32_t hysteresis;
    uint32_t tripId;     //<0     //supported in sa525m
    uint32_t tZoneId;    //<0    //supported in sa525m
    le_sls_Link_t link;
...
typedef struct
{
    ...
    char Type[50];  //length and possible negative return value
    uint32_t tripPointListSize;
...
{
    uint32_t cDevId; //<0
    uint32_t maxCoolingLevel;    //<0
    uint32_t currentCoolingLevel;    //<0
    char description[50];   ////length and possible negative return value
    le_sls_Link_t link;
...
namespace telux {
    namespace tafsvc {
        ...
        class taf_ThermServiceListener : public telux::common::IServiceStatusListener {
        public:
        ...
        public:
            ...
            std::shared_ptr<telux::therm::IThermalListener> thermalListener;
            ...
            le_result_t GetBoundCoolingDeviceListSize(taf_therm_ThermalZoneRef_t, uint32_t* listSize);
            ...
            le_result_t GetThermalZonePassiveTemp(taf_therm_ThermalZoneRef_t, uint32_t* passiveTemp);
            ...
            le_result_t GetBoundTripPointThermalZoneID(taf_therm_TripPointRef_t, uint32_t* boundTZoneID);
            #endif
            
            le_result_t GetBoundTripPointType(taf_therm_TripPointRef_t, char*, size_t );
            le_result_t GetBoundTripPointThreshold(taf_therm_TripPointRef_t, uint32_t* boundThreshold);
            le_result_t GetBoundTripPointHysterisis(taf_therm_TripPointRef_t, uint32_t* boundHysterisis);
            le_result_t GetBoundCoolingId(taf_therm_BoundCoolingDeviceRef_t, uint32_t* boundCoolingId);
            le_result_t GetBoundTripPointListSize(taf_therm_BoundCoolingDeviceRef_t, uint32_t* listSize);
            ...
            le_result_t GetCoolingDeviceListSize(taf_therm_CoolingDeviceListRef_t, uint32_t* listSize);
            ...
            le_result_t GetCDevMaxCoolingLevel(taf_therm_CoolingDeviceRef_t, uint32_t* maxCoolingLevel);
            le_result_t GetCDevCurrentCoolingLevel(taf_therm_CoolingDeviceRef_t, uint32_t* currentCoolingLevel);
