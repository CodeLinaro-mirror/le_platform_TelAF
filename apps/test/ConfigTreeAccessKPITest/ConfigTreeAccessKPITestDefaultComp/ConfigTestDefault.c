 ...
 */
//--------------------------------------------------------------------------------------------------
static void ReadDefaultConfigIntNodeValue
(
 ...
 */
//--------------------------------------------------------------------------------------------------
static void WriteDefaultConfigIntNodeValue
(
    const char* treePath,      ///< Tree path + child node
 ...
 */
//--------------------------------------------------------------------------------------------------
static void ReadDefaultWriteIntegerConfigTreeKPI
(
)
{
    ...
    LE_TEST_INFO("ReadDefaultWriteIntegerConfigTreeKPI");
    ...
    WriteDefaultConfigIntNodeValue(CFG_READ_WRITE_PATH,valueSet);
    ...
    ReadDefaultConfigIntNodeValue(CFG_READ_WRITE_PATH,&valueGet2);
    LE_TEST_OK(valueSet == valueGet2,"***ReadDefaultWriteIntegerConfigTreeKPI***-LE_OK");
 ...
 */
//--------------------------------------------------------------------------------------------------
static void TestDefaultImportReadWrite(const char* treePath)
{
    int32_t valueSet = 2025;
    int32_t value;

   //Writing into the imported config tree path
    LE_TEST_INFO("TestDefaultImportReadWrite");
    le_cfg_IteratorRef_t iteratorRef = le_cfg_CreateWriteTxn(treePath);
    if (iteratorRef == NULL)
    {
        LE_TEST_INFO("Failed to create write transaction");
   ...
   //Reading from the imported config tree path
    ...
    LE_TEST_OK(value == valueSet,"***TestDefaultImportReadWrite***-LE_OK");
    le_cfg_CancelTxn(iteratorRef);
}
 ...
 */
//--------------------------------------------------------------------------------------------------
static void DefaultClearTree(const char* testRoot)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static void TestDefaultImportJSON(const char* testRoot, bool isSuccess)
{
    ...
    snprintf(filePath,
    150, "/legato/systems/current/appsWriteable/ConfigTreeAccessKPIAppDefaultTest/data/configKPITest-%s.json", testRoot);
    ...
    TestDefaultImportReadWrite(filePath);
    unlink(filePath);
}
 ...
 */
//--------------------------------------------------------------------------------------------------
static void TestDefaultImportKPI()
{
    LE_TEST_INFO("TestDefaultImportKPI");
    ...
    DefaultClearTree("100");
    TestDefaultImportJSON("100",true);
...
COMPONENT_INIT
{
    ...
    ReadDefaultWriteIntegerConfigTreeKPI();
    TestDefaultImportKPI();
